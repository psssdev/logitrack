...
export default function InicioPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setIsLoading(true);
      try {
        const { summary, topClients } = await getDashboardData();
        setSummary(summary);
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Distribuição de todas as encomendas por status.</CardDescription>
          </CardHeader>
              ...
              totalPie > 0 ? (
                <ChartContainer config={chartConfig} className="min-h-[250px] w-full max-w-sm">
                  <PieChart accessibilityLayer>
                    ...
                    <Pie data={chartData} dataKey="value" nameKey="name" innerRadius={60} outerRadius={100} paddingAngle={2} labelLine={false} label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`}>
                ...
                <div className="flex h-[250px] w-full flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm text-muted-foreground/80">Crie uma para ver o gráfico.</p>
                </div>
              )
            )}
          </CardContent>
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>Clientes que mais geraram faturamento.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                topClients.length > 0 ? (
                    <div className="rounded-md border">
                        <Table>
                            ...
                            <TableBody>
                                ...
                                <TableRow key={client.id}>
                                    ...
                                    <TableCell className="text-right font-semibold">{formatCurrency(client.totalValue)}</TableCell>
                                </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                     ...
                     <div className="flex h-48 flex-col items-center justify-center rounded-md border-2 border-dashed p-12 text-center">
                        <p className="text-muted-foreground">Nenhum cliente com encomendas.</p>
                        <p className="text-sm text-muted-foreground/80">Os dados aparecerão aqui quando houver encomendas.</p>
                    </div>
                )
             )}
          </CardContent>
