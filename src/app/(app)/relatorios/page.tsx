...
export default function RelatoriosPage() {
  ...
  const { monthlyData, paymentData, totalRevenue, totalReceivable, ticketMedio, driverPerformance, clientPerformance } = useMemo(() => {
    ...
    const totalRevenueValue = orders.reduce((sum, o) => sum + o.valorEntrega, 0);
    const totalReceivableValue = orders.filter(o => o.pago === false).reduce((sum, o) => {
        const paidAmount = o.payments?.reduce((s, p) => s + p.amount, 0) || 0;
        return sum + (o.valorEntrega - paidAmount);
    }, 0);
    const ticketMedioValue = orders.length > 0 ? totalRevenueValue / orders.length : 0;
       ...
       <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Saldo devedor de encomendas pendentes.</CardDescription>
          </CardHeader>
          <CardContent>
