...
function AddLocationDialog({
    ...
    const form = useForm<NewLocation>({
        ...
        defaultValues: { storeId: '', name: '', logradouro: '', numero: '', bairro: '', cidade: '', estado: '', cep: '' },
    });

    // Set storeId when store is selected
    React.useEffect(() => {
        if (selectedStore?.id) {
    ...
    async function onSubmit(data: NewLocation) {
        if (!firestore || !selectedStore) return;
...
export function NewFinancialEntryForm({ vehicles, clients, origins: initialOrigins, destinations: initialDestinations }: { vehicles: Vehicle[], clients: Client[], origins: Origin[], destinations: Destino[] }) {
    ...
    const relevantSalesQuery = useMemoFirebase(() => {
        if (!firestore || !selectedVehicleId || !selectedStore) return null;
        return query(
            collection(firestore, 'stores', selectedStore.id, 'financialEntries'),
            where('vehicleId', '==', selectedVehicleId)
        );
    }, [firestore, selectedVehicleId, selectedStore]);
    ...
    React.useEffect(() => {
        async function fetchClientAddressesAndSuggestOrigin() {
            if (!selectedClientId || liveOrigins.length === 0 || !firestore || !selectedStore) {
                setSuggestMeta(null);
                return;
            };
            const clientRef = doc(firestore, 'stores', selectedStore.id, 'clients', selectedClientId);
